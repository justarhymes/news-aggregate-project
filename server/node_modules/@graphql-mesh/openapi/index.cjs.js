...
async function getValidOAS3(spec, options) {
    ...
    if (typeof spec.swagger === 'string' && spec.swagger === '2.0') {
        ...
        if (!options.skipSchemaValidation) {
            ...
            const valid = await oasValidator.validate(spec, {});
            if (!valid) {
...
const translationLog$1 = debug.debug('translation');
const httpLog$1 = debug.debug('http');
...
function graphQLErrorWithExtensions(message, extensions) {
    return new graphql.GraphQLError(message, null, null, null, null, null, extensions);
}
...
// Copyright IBM Corp. 2018. All Rights Reserved.
const preprocessingLog$1 = debug__default('preprocessing');
/**
...
const translationLog$2 = debug__default('translation');
/**
...
function createOrReuseOt({ def, operation, data, iteration, isInputObjectType, }) {
    ...
    if (!isInputObjectType) {
        translationLog$2(`Create object type '${def.graphQLTypeName}'` +
            (typeof operation === 'object' ? ` (for operation '${operation.operationString}')` : ''));
        def.graphQLType = new graphql.GraphQLObjectType({
        ...
        translationLog$2(`Create input object type '${def.graphQLInputObjectTypeName}'` +
            (typeof operation === 'object' ? ` (for operation '${operation.operationString}')` : ''));
        def.graphQLInputObjectType = new graphql.GraphQLInputObjectType({
...
function createOrReuseUnion({ def, operation, data, iteration }) {
    ...
    if (typeof def.graphQLType !== 'undefined') {
        ...
        def.graphQLType = new graphql.GraphQLUnionType({
            name: def.graphQLTypeName,
            description,
            types,
            resolveType: source => {
...
function createOrReuseList({ def, operation, iteration, isInputObjectType, data, }) {
    ...
    if (itemsType !== null) {
        const listObjectType = new graphql.GraphQLList(itemsType);
...
function createOrReuseEnum({ def }) {
    ...
    if (def.graphQLType && typeof def.graphQLType !== 'undefined') {
        ...
        def.graphQLType = new graphql.GraphQLEnumType({
            name: def.graphQLTypeName,
            values,
        });
        return def.graphQLType;
...
function getScalarType({ def }) {
    switch (def.targetGraphQLType) {
        case 'id':
            def.graphQLType = graphql.GraphQLID;
            break;
        case 'string':
            def.graphQLType = graphql.GraphQLString;
            break;
        case 'integer':
            def.graphQLType = graphql.GraphQLInt;
            break;
        case 'number':
            def.graphQLType = graphql.GraphQLFloat;
            break;
        case 'float':
            def.graphQLType = graphql.GraphQLFloat;
            break;
        case 'boolean':
            def.graphQLType = graphql.GraphQLBoolean;
            break;
        case 'json':
            def.graphQLType = graphqlScalars.GraphQLJSON;
            break;
        default:
...
function createFields({ def, links, operation, data, iteration, isInputObjectType, }) {
    ...
    for (const fieldTypeKey in fieldTypeDefinitions) {
        ...
        if (objectType) {
            ...
            fields[sanePropName] = {
                type: requiredProperty ? new graphql.GraphQLNonNull(objectType) : objectType,
...
function getArgs({ requestPayloadDef, parameters, operation, data }) {
    ...
    if (typeof requestPayloadDef === 'object') {
        ...
        const reqRequired = typeof operation === 'object' && typeof operation.payloadRequired === 'boolean'
            ...
            : false;
        args[saneName] = {
            type: reqRequired ? new graphql.GraphQLNonNull(reqObjectType) : reqObjectType,
...
// Copyright IBM Corp. 2018. All Rights Reserved.
const translationLog$3 = debug__default('translation');
/**
...
const getViewerOT = (name, protocolName, securityType, queryFields, data) => {
    ...
    if (typeof scheme === 'object') {
        for (const parameterName in scheme.parameters) {
            args[parameterName] = { type: new graphql.GraphQLNonNull(graphql.GraphQLString) };
        }
    }
...
// Copyright IBM Corp. 2018. All Rights Reserved.
const translationLog$4 = debug__default('translation');
/**
...
const handler = {
    async getMeshSource({ config, cache }) {
        ...
        const spec = await utils.readFileOrUrlWithCache(path, cache, {
            headers: config.schemaHeaders,
        });
        const fetch = (...args) => fetchache.fetchache(args[0] instanceof fetchache.Request ? args[0] : new fetchache.Request(...args), cache);
        const headersFactory = utils.getInterpolatedHeadersFactory(config.operationHeaders);
        const { schema } = await createGraphQLSchema(spec, {
            ...
            resolverMiddleware: (getResolverParams, originalFactory) => (root, args, context, info) => {
                ...
                resolverParams.requestOptions = {
                    headers: utils.getHeadersObject(headersFactory(resolverData)),
                };
                return originalFactory(() => resolverParams)(root, args, context, info);
            },
        });
        const { args, contextVariables } = utils.parseInterpolationStrings(Object.values(config.operationHeaders || {}));
        const rootFields = [
