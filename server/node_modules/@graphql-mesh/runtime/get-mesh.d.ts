import { GraphQLSchema, DocumentNode, GraphQLError } from 'graphql';
import { ExecuteMeshFn, GetMeshOptions, Requester, ResolvedTransform, SubscribeMeshFn } from './types';
import { Hooks, KeyValueCache, RawSourceOutput, MeshTransform } from '@graphql-mesh/types';
export declare function groupTransforms({ transforms, apiName, cache, hooks, }: {
    transforms: ResolvedTransform[];
    apiName: string;
    cache: KeyValueCache;
    hooks: Hooks;
}): {
    wrapTransforms: MeshTransform[];
    noWrapTransforms: MeshTransform[];
};
export declare function getMesh(options: GetMeshOptions): Promise<{
    execute: ExecuteMeshFn;
    subscribe: SubscribeMeshFn;
    schema: GraphQLSchema;
    rawSources: RawSourceOutput[];
    sdkRequester: Requester;
    contextBuilder: (initialContextValue?: any) => Promise<Record<string, any>>;
    destroy: () => void;
    hooks: Hooks;
    cache: KeyValueCache;
}>;
export declare class GraphQLMeshSdkError<Data = any, Variables = any> extends Error {
    errors: ReadonlyArray<GraphQLError>;
    document: DocumentNode;
    variables: Variables;
    data: Data;
    constructor(errors: ReadonlyArray<GraphQLError>, document: DocumentNode, variables: Variables, data: Data);
}
